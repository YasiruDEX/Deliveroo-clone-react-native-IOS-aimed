...
export { DEFAULT_MAX_FIELD_DEPTH, resolveSearchConfig, resolveSearchConfigForBaseFieldPaths } from '../_chunks/resolve-67bA2d6j.js';
const HELP_IDS = {
...
function createTypeWithMembersProblemsAccessor(memberPropertyName) {
  let getMembers = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : type => get(type, memberPropertyName);
...
function validateComponent(typeDef) {
  ...
  if (components.input && !isComponentLike(components.input)) {
    warnings.push(warning("The `components.input` property is set but does not appear to be a valid React component (expected a function, but saw ".concat(inspect(components.input), "). If you have imported a custom input component, please verify that you have imported the correct named/default export.")));
  }
  if (components.field && !isComponentLike(components.field)) {
    warnings.push(warning("The `components.field` property is set but does not appear to be a valid React component (expected a function, but saw ".concat(inspect(components.field), "). If you have imported a custom field component, please verify that you have imported the correct named/default export.")));
  }
  if (components.item && !isComponentLike(components.item)) {
    warnings.push(warning("The `components.item` property is set but does not appear to be a valid React component (expected a function, but saw ".concat(inspect(components.item), "). If you have imported a custom item component, please verify that you have imported the correct named/default export.")));
  }
  if (components.preview && !isComponentLike(components.preview)) {
    warnings.push(warning("The `components.preview` property is set but does not appear to be a valid React component (expected a function, but saw ".concat(inspect(components.preview), "). If you have imported a custom preview component, please verify that you have imported the correct named/default export.")));
  }
  return warnings;
}
...
function validateFieldName(name) {
  if (typeof name !== "string") {
    return [error('Field names must be strings. Saw "'.concat(inspect(name), '"'), HELP_IDS.OBJECT_FIELD_NAME_INVALID)];
  }
...
function validateField(field, _visitorContext) {
  if (!isPlainObject(field)) {
    return [error("Incorrect type for field definition - should be an object, saw ".concat(inspect(field)), HELP_IDS.OBJECT_FIELD_DEFINITION_INVALID_TYPE)];
  }
  const problems = [];
...
var reference = (typeDef, visitorContext) => {
  ...
  const problems = flatten([isValidTo ? getDupes(normalizedTo, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, ' members with same type, but not unique names "').concat(dupes[0].type, '" in reference. This makes it impossible to tell their values apart and you should consider naming them'), HELP_IDS.REFERENCE_TO_INVALID)) : error('The reference type is missing or having an invalid value for the required "to" property. It should be an array of accepted types.', HELP_IDS.REFERENCE_TO_INVALID)]);
...
function format(value) {
  ...
  if (typeof value === "object" && value !== null) {
    return "object with keys ".concat(humanizeList(Object.keys(value).map(quote$2)));
  }
...
var array = (typeDef, visitorContext) => {
  ...
  const problems = flatten([ofIsArray ? getDupes(typeDef.of, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, ' members with same type, but not unique names "').concat(dupes[0].type, '" in array. This makes it impossible to tell their values apart and you should consider naming them'), HELP_IDS.ARRAY_OF_NOT_UNIQUE)) : error('The array type is missing or having an invalid value for the required "of" property', HELP_IDS.ARRAY_OF_INVALID)]);
  ...
  const [primitiveTypes, objectTypes] = partition(of, ofType => {
    var _a2;
  ...
  if (isMixedArray) {
    ...
    problems.push(error("The array type's 'of' property can't have both object types and primitive types (found primitive type ".concat(pluralize(primitiveTypeNames), " ").concat(humanizeList(primitiveTypeNames.map(quote$2)), " and object type").concat(pluralize(objectTypeNames), " ").concat(humanizeList(objectTypeNames.map(quote$2)), ")"), HELP_IDS.ARRAY_OF_INVALID));
  }
  ...
  if (!isMixedArray && Array.isArray(list)) {
    ...
    if (isArrayOfPrimitives) {
      list.forEach(option => {
        ...
        if (!isDeclared) {
          const formattedTypeList = humanizeList(primitiveTypes.map(t => t.name || t.type), {
            conjunction: "or"
          });
      ...
      list.forEach(option => {
        ...
        if (!isDeclared) {
          problems.push(error("An invalid entry found in options.list: ".concat(format(option), '. Must be an object with "_type" set to ').concat(humanizeList(objectTypes.map(t => t.name || t.type).map(t => t === "object" ? "undefined" : quote$2(t)), {
            conjunction: "or"
...
function validateBlockType(typeDef, visitorContext) {
  ...
  if (disallowedKeys.length > 0) {
    problems.push(error("Found unknown properties for block declaration: ".concat(humanizeList(disallowedKeys.map(quote$1)))));
  }
  if (marks) {
...
function validateMarks(marks, visitorContext, problems) {
  ...
  if (disallowedMarkKeys.length > 0) {
    problems.push(error("Found unknown properties for block declaration: ".concat(humanizeList(disallowedMarkKeys.map(quote$1)))));
  }
...
function validateLists(lists, visitorContext, problems) {
  ...
  lists.forEach((list, index) => {
    ...
    if (disallowedKeys.length > 0) {
      problems.push(error("Found unknown properties for list ".concat(name, ": ").concat(humanizeList(disallowedKeys.map(quote$1)))));
    }
    if (!list.value) {
...
function validateStyles(styles, visitorContext, problems) {
  ...
  styles.forEach((style, index) => {
    ...
    if (disallowedKeys.length > 0) {
      problems.push(error("Found unknown properties for style ".concat(name, ": ").concat(humanizeList(disallowedKeys.map(quote$1)))));
    }
    if (!style.value) {
...
function validateDecorators(decorators, visitorContext, problems) {
  decorators.forEach((decorator, index) => {
    ...
    if (disallowedKeys.length > 0) {
      problems.push(error("Found unknown properties for decorator ".concat(name, ": ").concat(humanizeList(disallowedKeys.map(quote$1)))));
    }
    if (!decorator.value) {
...
function validateTypeName(typeName, visitorContext) {
  ...
  if (!typeName) {
    return [error("Type is missing a type. Valid types are: ".concat(humanizeList(possibleTypeNames)), HELP_IDS.TYPE_MISSING_TYPE)];
  }
  if (typeof typeName !== "string") {
    return [error('Type has an invalid "type"-property - should be a string. Valid types are: '.concat(humanizeList(possibleTypeNames)), HELP_IDS.TYPE_MISSING_TYPE)];
  }
  ...
  if (!isValid) {
    ...
    const suggestion = suggestions.length > 0 ? " Did you mean ".concat(humanizeList(suggestions.map(quote), {
      conjunction: "or"
    }), "?") : "";
    return [error("Unknown type: ".concat(typeName, ".").concat(suggestion, "\n\nValid types are: ").concat(humanizeList(possibleTypeNames)))];
  }
  return [];
}
...
var crossDatasetReference = (typeDef, visitorContext) => {
  ...
  const problems = flatten([isValidTo ? getDupes(normalizedTo, t => "".concat(t.name, ";").concat(t.type)).map(dupes => error("Found ".concat(dupes.length, ' members with same type, but not unique names "').concat(dupes[0].type, '" in reference. This makes it impossible to tell their values apart and you should consider naming them'), HELP_IDS.CROSS_DATASET_REFERENCE_INVALID)) : error('The cross dataset reference type is missing or having an invalid value for the required "to" property. It should be an array of accepted types.', HELP_IDS.CROSS_DATASET_REFERENCE_INVALID)]);
  if (isValidTo && normalizedTo.length === 0) {
