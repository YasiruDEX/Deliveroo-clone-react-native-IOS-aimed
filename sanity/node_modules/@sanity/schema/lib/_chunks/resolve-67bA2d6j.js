...
function getCachedStringFieldPaths(type, maxDepth) {
  ...
  if (!type[symbol]) {
    type[symbol] = uniqBy([...BASE_WEIGHTS, ...deriveFromPreview(type, maxDepth), ...getStringFieldPaths(type, maxDepth).map(path => ({
      weight: 1,
      path
...
function getCachedBaseFieldPaths(type, maxDepth) {
  const symbol = getStringFieldSymbol(maxDepth);
  if (!type[symbol]) {
    type[symbol] = uniqBy([...BASE_WEIGHTS, ...deriveFromPreview(type, maxDepth)], spec => spec.path.join("."));
  }
  return type[symbol];
}
...
export { DEFAULT_MAX_FIELD_DEPTH, resolveSearchConfig, resolveSearchConfigForBaseFieldPaths };
//# sourceMappingURL=resolve-67bA2d6j.js.map
