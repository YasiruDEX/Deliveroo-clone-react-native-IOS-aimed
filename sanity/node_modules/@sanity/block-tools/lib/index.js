...
uniq__default.default(Object.values(ELEMENT_MAP).filter(tag => "style" in tag).map(tag => tag.style));
uniq__default.default(Object.values(HTML_DECORATOR_TAGS));
const _XPathResult = {
  ANY_TYPE: 0,
  NUMBER_TYPE: 1,
  STRING_TYPE: 2,
...
function trimWhitespace(blocks) {
  blocks.forEach(block => {
    ...
    block.children.forEach((child, index) => {
      ...
      if (prevChild && isEqual__default.default(prevChild.marks, child.marks) && isWhiteSpaceChar(child.text)) {
        prevChild.text += " ";
        block.children.splice(index, 1);
      } else if (nextChild && isEqual__default.default(nextChild.marks, child.marks) && isWhiteSpaceChar(child.text)) {
      ...
      if (this._markDefs.length > 0) {
        blocks.filter(block => block._type === "block").forEach(block => {
          ...
          block.markDefs = block.markDefs.concat(this._markDefs.filter(def => {
            return flatten__default.default(block.children.map(child => child.marks || [])).includes(def._key);
          }));
        });
      }
...
function normalizeBlock(node) {
  ...
  block.children = block.children.reduce((acc, child) => {
    ...
    if (previousChild && isPortableTextSpan(child) && isPortableTextSpan(previousChild) && isEqual__default.default(previousChild.marks, child.marks)) {
