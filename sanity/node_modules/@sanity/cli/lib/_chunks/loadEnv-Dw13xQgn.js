...
var require$$1 = require('get-it');
var require$$2 = require('get-it/middleware');
...
var path__default$1 = /*#__PURE__*/_interopDefaultCompat(path$2);
...
function defineHttpRequest(envMiddleware, _ref) {
  ...
  const request = require$$1.getIt([maxRetries > 0 ? require$$2.retry({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    retryDelay,
    // This option is typed incorrectly in get-it.
    maxRetries,
    shouldRetry
  }) : {}, ...envMiddleware, printWarnings, require$$2.jsonRequest(), require$$2.jsonResponse(), require$$2.progress(), httpError, require$$2.observable({
...
function shouldRetry(err, attempt, options) {
  ...
  return require$$2.retry.shouldRetry(err, attempt, options);
}
  ...
  return new Observable(observer => {
    ...
    async function getEventSource() {
      const {
        ...
        return require('./node-RY2LZsjz.js');
      }).then(function (n) {
        return n.node;
      });
...
var version$2 = "6.10.0";
const middleware = [require$$2.debug({
  verbose: true,
...
function isSanityV2StudioRoot(basePath) {
  try {
    const content = require$$0__default$3.default.readFileSync(path__default.default.join(basePath, "sanity.json"), "utf8");
...
function fileExists(filePath) {
  return require$$0__default$3.default.existsSync(filePath);
}
var main$1 = {
  exports: {}
};
...
function loadEnv(mode, envDir) {
  ...
  const parsed = Object.fromEntries(envFiles.flatMap(file => {
    ...
    if (!envPath) return [];
    return Object.entries(parse_1(fs__default.default.readFileSync(envPath)));
  }));
...
function lookupFile(dir, formats, options) {
  for (const format of formats) {
    const fullPath = path__default$1.default.join(dir, format);
    if (fs__default.default.existsSync(fullPath) && fs__default.default.statSync(fullPath).isFile()) {
      return fullPath;
    }
  }
  const parentDir = path__default$1.default.dirname(dir);
...
//# sourceMappingURL=loadEnv-Dw13xQgn.js.map
