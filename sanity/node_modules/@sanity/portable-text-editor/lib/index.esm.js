...
import React, { createContext, useContext, useMemo, useEffect, useRef, useCallback, useState, startTransition, useLayoutEffect, forwardRef } from 'react';
import { Subject } from 'rxjs';
import { Schema } from '@sanity/schema';
import debug$l from 'debug';
import { randomKey } from '@sanity/util/content';
import { ReactEditor, withReact, Slate, useSlate, useSelected, useSlateStatic, Editable } from 'slate-react';
import { Element as Element$1, Text, Editor, Transforms, Range, Path, Node, Operation, createEditor } from 'slate';
...
function debugWithName(name) {
  ...
  if (debug$l && debug$l.enabled(namespace)) {
    return debug$l(namespace);
  }
  return debug$l(rootName);
}
const defaultKeyGenerator = () => randomKey(12);
const PortableTextEditorKeyGeneratorContext = createContext(defaultKeyGenerator);
  ...
  if (value && Array.isArray(value)) {
    return value.map(block => {
      ...
      if (isPortableText) {
        ...
        if (!hasMissingStyle && !hasMissingMarkDefs && !hasMissingChildren && !hasInlines && Element$1.isElement(block)) {
          return block;
        }
        if (hasMissingStyle) {
...
function isEqualToEmptyEditor(children, schemaTypes) {
  var _a;
  return children === void 0 || children && Array.isArray(children) && children.length === 0 || children && Array.isArray(children) && children.length === 1 && Element$1.isElement(children[0]) && children[0]._type === schemaTypes.block.name && "style" in children[0] && children[0].style === schemaTypes.styles[0].value && !("listItem" in children[0]) && Array.isArray(children[0].children) && children[0].children.length === 1 && Text.isText(children[0].children[0]) && children[0].children[0]._type === "span" && !((_a = children[0].children[0].marks) == null ? void 0 : _a.join("")) && children[0].children[0].text === "";
}
...
const debug$j = debugWithName("plugin:withPortableTextMarkModel");
function createWithPortableTextMarkModel(types, change$) {
  return function withPortableTextMarkModel(editor) {
    ...
    editor.normalizeNode = nodeEntry => {
      ...
      if (isSpan || isTextBlock) {
        if (isSpan && !Array.isArray(node.marks)) {
          debug$j("Adding .marks to span node");
        ...
        for (const op of editor.operations) {
          if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
            ...
            debug$j("Copying markDefs over to merged block", op);
          ...
          if (op.type === "split_node" && op.path.length === 1 && Element$1.isElementProps(op.properties) && op.properties._type === types.block.name && "markDefs" in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] + 1 < editor.children.length) {
            const [targetBlock, targetPath] = Editor.node(editor, [op.path[0] + 1]);
            debug$j("Copying markDefs over to split block", op);
            if (editor.isTextBlock(targetBlock)) {
              const oldDefs = Array.isArray(targetBlock.markDefs) && targetBlock.markDefs || [];
              Transforms.setNodes(editor, {
      ...
      if (op.type === "insert_text") {
        ...
        if (selection && Range.isCollapsed(selection) && ((_b = (_a = Editor.marks(editor)) == null ? void 0 : _a.marks) == null ? void 0 : _b.some(mark => !decorators.includes(mark)))) {
          ...
          if (Text.isText(node) && node.text.length === selection.focus.offset && Array.isArray(node.marks) && node.marks.length > 0) {
            ...
            debug$j("Inserting text at end of annotation");
            return;
          }
        }
      }
    ...
    editor.pteToggleMark = mark => {
      ...
      if (isActive) {
        debug$j("Remove mark '".concat(mark, "'"));
        Editor.removeMark(editor, mark);
      } else {
        debug$j("Add mark '".concat(mark, "'"));
    ...
    for (const [block, path] of blocks) {
      if (editor.isTextBlock(block)) {
        ...
        if (!isEqual(newMarkDefs, block.markDefs)) {
          debug$j("Removing markDef not in use");
          Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
            at: path
    ...
    portableTextEditor.setEditable({
      ...
      insertChild: (type, value) => {
        ...
        if (isSpanNode && focusNode._type !== types.span.name) {
          debug$i("Inserting span child next to inline object child, moving selection + 1");
          editor.move({
            distance: 1,
            unit: "character"
          });
        ...
        if (selection) {
          ...
          if (range) {
            if (!(options == null ? void 0 : options.mode) || (options == null ? void 0 : options.mode) === "selected") {
              debug$i("Deleting content in selection");
            ...
            if ((options == null ? void 0 : options.mode) === "blocks") {
              debug$i("Deleting blocks touched by selection");
              Transforms.removeNodes(editor, {
                at: range,
                voids: true,
                match: node => {
            ...
            if ((options == null ? void 0 : options.mode) === "children") {
              debug$i("Deleting children touched by selection");
              Transforms.removeNodes(editor, {
                at: range,
                voids: true,
                match: node => {
      ...
      removeAnnotation: type => {
        ...
        debug$i("Removing annotation", type);
        if (selection) {
          if (Range.isCollapsed(selection)) {
...
const debug$h = debugWithName("applyPatches");
const debugVerbose$3 = debug$h.enabled && true;
function createApplyPatch(schemaTypes) {
  ...
  return function (editor, patch) {
    ...
    if (debugVerbose$3) {
      debug$h("\n\nNEW PATCH =============================================================");
      debug$h(JSON.stringify(patch, null, 2));
    }
    try {
      switch (patch.type) {
        case "insert":
...
function diffMatchPatch(editor, patch) {
  ...
  if (!block) {
    debug$h("Block not found");
    return false;
  }
  if (!child || !childPath) {
    debug$h("Child not found");
    return false;
  }
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length > 1 && patch.path[1] !== "children") {
    debug$h("Ignoring patch targeting void value");
    return false;
  }
  if (patch.path.length === 1) {
    ...
    debug$h("Inserting blocks at path [".concat(normalizedIdx2, "]"));
    debugState(editor, "before");
  ...
  debug$h("Inserting children at path ".concat(childInsertPath));
  debugState(editor, "before");
...
function setPatch(editor, patch) {
  ...
  if (isTextBlock && patch.path.length > 1 && patch.path[1] !== "children") {
    debug$h("Ignoring setting void value");
    return false;
  }
  debugState(editor, "before");
  if (isTextBlock && child && childPath) {
    if (Text.isText(value) && Text.isText(child)) {
      ...
      debug$h("Setting non-text property");
      editor.apply({
        type: "set_node",
        path: childPath,
        properties: {},
...
function unsetPatch(editor, patch, previousPatch) {
  ...
  if (patch.path.length === 1) {
    ...
    debug$h("Removing block at path [".concat(blockIndex, "]"));
    debugState(editor, "before");
    Transforms.removeNodes(editor, {
      at: [blockIndex]
    });
  ...
  if (editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3) {
    ...
    debug$h("Unsetting child at path ".concat(JSON.stringify(childPath)));
    debugState(editor, "before");
    if (debugVerbose$3) {
      debug$h("Removing child at path ".concat(JSON.stringify([childPath, childIndex])));
    }
    Transforms.removeNodes(editor, {
      at: childPath
    });
...
function debugState(editor, stateName) {
  ...
  debug$h("Children ".concat(stateName, ":"), JSON.stringify(editor.children, null, 2));
  debug$h("Selection ".concat(stateName, ": "), JSON.stringify(editor.selection, null, 2));
}
...
const debug$g = debugWithName("plugin:withUndoRedo");
const debugVerbose$2 = debug$g.enabled && false;
  ...
  return editor => {
    ...
    if (patches$) {
      editor.subscriptions.push(() => {
        ...
        const sub = patches$.subscribe(_ref5 => {
          let {
            patches,
            snapshot
          } = _ref5;
          ...
          patches.forEach(patch => {
            if (!reset && patch.origin !== "local" && remotePatches) {
              if (patch.type === "unset" && patch.path.length === 0) {
                debug$g("Someone else cleared the content, resetting undo/redo history");
                editor.history = {
                  undos: [],
                  redos: []
                };
    ...
    editor.apply = op => {
      ...
      if (save) {
        ...
        if (step && merge) {
          ...
          undos.push(newStep);
          debug$g("Created new undo step", step);
        }
    ...
    editor.undo = () => {
      ...
      if (undos.length > 0) {
        const step = undos[undos.length - 1];
        debug$g("Undoing", step);
        if (step.operations.length > 0) {
          const otherPatches = remotePatches.filter(item => item.time >= step.timestamp);
          let transformedOperations = step.operations;
          otherPatches.forEach(item => {
            transformedOperations = flatten$1(transformedOperations.map(op => transformOperation(editor, item.patch, op, item.snapshot, item.previousSnapshot)));
          });
          try {
            Editor.withoutNormalizing(editor, () => {
            ...
            debug$g("Could not perform undo step", err);
            remotePatches.splice(0, remotePatches.length);
            Transforms.deselect(editor);
            editor.history = {
              undos: [],
      ...
      if (redos.length > 0) {
        ...
        if (step.operations.length > 0) {
          ...
          try {
            ...
            debug$g("Could not perform redo step", err);
            remotePatches.splice(0, remotePatches.length);
            Transforms.deselect(editor);
            editor.history = {
              undos: [],
...
function transformOperation(editor, patch, operation, snapshot, previousSnapshot) {
  if (debugVerbose$2) {
    debug$g("Adjusting '".concat(operation.type, "' operation paths for '").concat(patch.type, "' patch"));
    debug$g("Operation ".concat(JSON.stringify(operation)));
    debug$g("Patch ".concat(JSON.stringify(patch)));
  }
  ...
  if (patch.type === "insert" && patch.path.length === 1) {
    ...
    debug$g("Adjusting block path (+".concat(patch.items.length, ") for '").concat(transformedOperation.type, "' operation and patch '").concat(patch.type, "'"));
  ...
  if (patch.type === "unset" && patch.path.length === 1) {
    ...
    if ("path" in transformedOperation && Array.isArray(transformedOperation.path) && transformedOperation.path[0] === unsetBlockIndex) {
      debug$g("Skipping transformation that targeted removed block");
      return [];
    }
    if (debugVerbose$2) {
      debug$g("Selection ".concat(JSON.stringify(editor.selection)));
      debug$g("Adjusting block path (-1) for '".concat(transformedOperation.type, "' operation and patch '").concat(patch.type, "'"));
    }
    return [adjustBlockPath(transformedOperation, -1, unsetBlockIndex)];
  }
  if (patch.type === "unset" && patch.path.length === 0) {
    debug$g("Adjusting selection for unset everything patch and ".concat(operation.type, " operation"));
    return [];
  }
  if (patch.type === "diffMatchPatch") {
    const operationTargetBlock = findOperationTargetBlock(editor, transformedOperation);
    if (!operationTargetBlock || !isEqual({
    ...
    diffPatches.forEach(diffPatch => {
      ...
      diffs.forEach((diff, index) => {
        ...
        if (diffType === DIFF_INSERT) {
          ...
          if (!diffs.slice(index).every(_ref6 => {
            let [dType] = _ref6;
...
function createWithPatches(_ref7) {
  let {
    change$,
    patches$,
    patchFunctions,
          ...
          withoutPatching(editor, () => {
            withoutSaving(editor, () => {
              withPreserveKeys(editor, () => {
                patches.forEach(patch => {
                  if (debug$f.enabled) debug$f("Handling remote patch ".concat(JSON.stringify(patch)));
...
function createWithPlaceholderBlock(_ref9) {
  let {
    schemaTypes,
    keyGenerator
  } = _ref9;
  return function withPlaceholderBlock(editor) {
    editor.createPlaceholderBlock = () => {
      debug$e("Creating placeholder block");
      return {
      ...
      for (const op of editor.operations) {
        if (op.type === "split_node" && op.path.length === 1 && editor.isTextBlock(op.properties) && op.properties.style !== defaultStyle && op.path[0] === path[0] && !Path.equals(path, op.path)) {
          ...
          if (Text.isText(child) && child.text === "") {
            debug$d("Normalizing split node to ".concat(defaultStyle, " style"), op);
            Transforms.setNodes(editor, {
              style: defaultStyle
            }, {
              at: [op.path[0] + 1],
    ...
    editor.pteToggleBlockStyle = blockStyle => {
      ...
      selectedBlocks.forEach(_ref10 => {
        ...
        if (editor.isTextBlock(node) && node.style === blockStyle) {
          debug$d("Unsetting block style '".concat(blockStyle, "'"));
          Transforms.setNodes(editor, {
            ...node,
            style: defaultStyle
          }, {
          ...
          if (blockStyle) {
            debug$d("Setting style '".concat(blockStyle, "'"));
          } else {
            debug$d("Setting default style", defaultStyle);
          }
          Transforms.setNodes(editor, {
            ...node,
...
const debug$c = debugWithName("plugin:withPortableTextLists");
...
function createWithPortableTextLists(types) {
  return function withPortableTextLists(editor) {
    editor.pteToggleListItem = listItemStyle => {
      ...
      if (isActive) {
        debug$c("Remove list item '".concat(listItemStyle, "'"));
        editor.pteUnsetListItem(listItemStyle);
      } else {
        debug$c("Add list item '".concat(listItemStyle, "'"));
    ...
    editor.pteUnsetListItem = listItemStyle => {
      ...
      selectedBlocks.forEach(_ref11 => {
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$c("Unsetting list '".concat(listItemStyle, "'"));
    ...
    editor.pteSetListItem = listItemStyle => {
      ...
      selectedBlocks.forEach(_ref12 => {
        ...
        debug$c("Setting list '".concat(listItemStyle, "'"));
        Transforms.setNodes(editor, {
          ...node,
          level: 1,
    ...
    editor.pteIncrementBlockLevels = reverse => {
      ...
      selectedBlocks.forEach(_ref14 => {
        ...
        if (editor.isListBlock(node)) {
          ...
          if (reverse) {
            level--;
            debug$c("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)));
          } else {
            level++;
            debug$c("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)));
          }
          Transforms.setNodes(editor, {
...
const debugVerbose$1 = debug$b.enabled && false;
  ...
  return function withPortableTextSelections(editor) {
    const emitPortableTextSelection = () => {
      if (prevSelection !== editor.selection) {
        ...
        if (debugVerbose$1) {
          debug$b("Emitting selection ".concat(JSON.stringify(ptRange || null), " (").concat(JSON.stringify(editor.selection), ")"));
        }
        if (ptRange) {
          change$.next({
            type: "selection",
...
function createWithHotkeys(types, keyGenerator, portableTextEditor, hotkeysFromOptions) {
  ...
  return function withHotKeys(editor) {
    editor.pteWithHotKeys = event => {
      ...
      if (isBackspace && editor.selection && editor.selection.focus.path[0] > 0 && Range.isCollapsed(editor.selection)) {
        ...
        const prevBlock = Node.descendant(editor, prevPath);
      ...
      if (isDelete && editor.selection && editor.selection.focus.offset === 0 && Range.isCollapsed(editor.selection) && editor.children[editor.selection.focus.path[0] + 1]) {
        const nextBlock = Node.descendant(editor, Path.next(editor.selection.focus.path.slice(0, 1)));
        const focusBlockPath = editor.selection.focus.path.slice(0, 1);
        const focusBlock = Node.descendant(editor, focusBlockPath);
      ...
      if ((isTab || isShiftTab) && editor.selection) {
        ...
        const [focusBlock] = isPortableTextSpan(focusChild) ? Editor.node(editor, editor.selection.focus, {
          depth: 1
        }) : [];
        const hasAnnotationFocus = focusChild && isPortableTextTextBlock(focusBlock) && isPortableTextSpan(focusChild) && (focusChild.marks || []).filter(m => (focusBlock.markDefs || []).map(def => def._key).includes(m)).length > 0;
        const [start] = Range.edges(editor.selection);
        const atStartOfNode = Editor.isStart(editor, start, start.path);
        if (focusChild && isPortableTextSpan(focusChild) && (!hasAnnotationFocus || atStartOfNode) && editor.pteIncrementBlockLevels(isShiftTab)) {
          event.preventDefault();
        }
      }
...
function validateValue(value, types, keyGenerator) {
  ...
  if (value.some((blk, index) => {
    ...
    if (blk._type === types.block.name) {
      ...
      const allUsedMarks = uniq(flatten$1(textBlock.children.filter(cld => cld._type === types.span.name).map(cld => cld.marks || [])));
...
function useSyncValue(props) {
  ...
  const updateValueFunctionRef = useRef();
  ...
  const updateValueDebounced = useCallback(debounce(updateFromCurrentValue, 1e3, {
    trailing: true,
    leading: false
              ...
              withoutPatching(slateEditor, () => {
                ...
                slateValueFromProps.forEach((currentBlock, currentBlockIndex) => {
                  ...
                  if (hasChanges && isValid) {
                    ...
                    if (validation.valid) {
                      if (oldBlock._key === currentBlock._key) {
                        if (debug$5.enabled) debug$5("Updating block", oldBlock, currentBlock);
                        _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex);
                      } else {
                        if (debug$5.enabled) debug$5("Replacing block", oldBlock, currentBlock);
                  ...
                  if (!oldBlock && isValid) {
                    const validationValue = [value[currentBlockIndex]];
                    const validation = validateValue(validationValue, schemaTypes, keyGenerator);
                    if (debug$5.enabled) debug$5("Validating and inserting new block in the end of the value", currentBlock);
                    if (validation.valid) {
...
function _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex) {
  ...
  if (slateEditor.isTextBlock(currentBlock) && slateEditor.isTextBlock(oldBlock)) {
    ...
    currentBlock.children.forEach((currentBlockChild, currentBlockChildIndex) => {
      ...
      const isTextChanged = !isEqual(currentBlockChild.text, oldBlockChild == null ? void 0 : oldBlockChild.text);
...
const debugVerbose = debug$4.enabled && false;
...
function Synchronizer(props) {
  ...
  useEffect(() => {
    ...
    const sub = change$.subscribe(next => {
      switch (next.type) {
        ...
        case "selection":
          startTransition(() => {
            if (debugVerbose) debug$4("Setting selection");
  ...
  return /* @__PURE__ */jsx(PortableTextEditorKeyGeneratorContext.Provider, {
    value: keyGenerator,
    children: /* @__PURE__ */jsx(PortableTextEditorContext.Provider, {
...
class PortableTextEditor extends React.Component {
  ...
  render() {
    ...
    return /* @__PURE__ */jsx(SlateContainer, {
      keyGenerator,
      maxBlocks,
      patches$: _patches$,
      portableTextEditor: this,
      readOnly,
      children: /* @__PURE__ */jsx(Synchronizer, {
        change$,
        keyGenerator,
        onChange,
        portableTextEditor: this,
...
const Leaf = props => {
  ...
  const path = useMemo(() => block ? [{
    _key: block == null ? void 0 : block._key
  }, "children", {
    _key: leaf._key
  }] : [], [block, leaf._key]);
  const decoratorValues = useMemo(() => schemaTypes.decorators.map(dec => dec.value), [schemaTypes.decorators]);
  const marks = useMemo(() => uniq((leaf.marks || EMPTY_MARKS).filter(mark => decoratorValues.includes(mark))), [decoratorValues, leaf.marks]);
  const annotationMarks = Array.isArray(leaf.marks) ? leaf.marks : EMPTY_MARKS;
  const annotations = useMemo(() => annotationMarks.map(mark => {
    var _a;
  ...
  useEffect(() => {
    ...
    if (sel && isEqual(sel.focus.path, path) && PortableTextEditor.isCollapsedSelection(portableTextEditor)) {
      startTransition(() => {
        setFocused(true);
      });
    }
  ...
  useEffect(() => {
    ...
    const sub = portableTextEditor.change$.subscribe(next => {
      ...
      if (next.type === "selection") {
        if (next.selection && isEqual(next.selection.focus.path, path) && PortableTextEditor.isCollapsedSelection(portableTextEditor)) {
          setFocused(true);
        } else {
          setFocused(false);
        }
  ...
  const content = useMemo(() => {
    ...
    if (Text.isText(leaf) && leaf._type === schemaTypes.span.name) {
      ...
      if (block && renderChild) {
        ...
        if (child) {
          const defaultRendered = /* @__PURE__ */jsx(Fragment, {
            children: returnedChildren
          });
          const _props = Object.defineProperty({
            annotations,
...
const Element = _ref18 => {
  ...
  const value = useMemo(() => fromSlateValue([element], schemaTypes.block.name, KEY_TO_VALUE_ELEMENT.get(editor))[0], [editor, element, schemaTypes.block.name]);
      ...
      return /* @__PURE__ */jsxs("span", {
        ...
        children: [children, /* @__PURE__ */jsxs("span", {
          ...
          children: [renderChild && renderChild({
            ...
            children: /* @__PURE__ */jsx(DefaultObject, {
              value
            }),
            ...
            type: schemaType2,
            value
          }), !renderChild && /* @__PURE__ */jsx(DefaultObject, {
            value
          })]
        }, element._key)]
      });
...
const PortableTextEditable = forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const slateEditor = useSlate();
  const blockTypeName = schemaTypes.block.name;
  const withInsertData = useMemo(() => createWithInsertData(change$, schemaTypes, keyGenerator), [change$, keyGenerator, schemaTypes]);
  const withHotKeys = useMemo(() => createWithHotkeys(schemaTypes, keyGenerator, portableTextEditor, hotkeys), [hotkeys, keyGenerator, portableTextEditor, schemaTypes]);
  useMemo(() => {
    if (readOnly) {
  ...
  const renderLeaf = useCallback(lProps => {
    const rendered = /* @__PURE__ */jsx(Leaf, {
      ...lProps,
      schemaTypes,
      renderAnnotation,
      renderChild,
  ...
  return hasInvalidValue ? null : /* @__PURE__ */jsx(Editable, {
    ...restProps,
    autoFocus: false,
...
export { PortableTextEditable, PortableTextEditor, compactPatches, defaultKeyGenerator as keyGenerator, usePortableTextEditor, usePortableTextEditorSelection };
