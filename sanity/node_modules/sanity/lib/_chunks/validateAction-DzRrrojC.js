...
function validateDocuments(options) {
  ...
  const workerPath = path.join(path.dirname(rootPkgPath), "lib", "_internal", "cli", "threads", "validateDocuments.js");
...
const summary = function (_ref2) {
  ...
  return ["".concat(logSymbols.success, " Valid:    ").concat(count(valid.documents, "documents")), "".concat(logSymbols.error, " Errors:   ").concat(count(errors.documents, "documents"), ", ").concat(count(errors.markers, "errors")), levelValue >= levelValues.warning && "".concat(logSymbols.warning, " Warnings: ").concat(count(warnings.documents, "documents"), ", ").concat(count(warnings.markers, "warnings")), levelValue >= levelValues.info && "".concat(logSymbols.info, " Info:     ").concat(count(infos.documents, "documents"), ", ").concat(count(infos.documents, "markers"))].filter(Boolean).join("\n");
};
const levelHeaders = {
...
const formatRootErrors = (root, hasChildren, paddingLength) => {
  ...
  const firstLine = "".concat(firstElbow, " (root) ").concat(firstPadding, " ").concat(logSymbols[first.level], " ").concat(first.message);
...
function formatDocumentValidation(_ref5) {
  ...
  const documentTypeHeader = isTty ? chalk.bgWhite(" ".concat(documentType, " ")) : "[".concat(documentType, "]");
...
async function validateAction(args, _ref9) {
  ...
  if (!unattendedMode) {
    output.print("".concat(chalk.yellow("".concat(logSymbols.warning, " Warning:")), " This command downloads all documents from a ") + "dataset and processes them through your local schema within a simulated browser environment.\n");
...
//# sourceMappingURL=validateAction-DzRrrojC.js.map
