...
function renderDocumentFromWorkerData() {
  ...
  if ((_a = workerData) == null ? void 0 : _a.dev) {
    global.__DEV__ = true;
  }
...
function sanityFaviconsPlugin(_ref3) {
  ...
  async function getFavicons() {
    ...
    cache.favicons = await fs$1.readdir(defaultFaviconsPath);
    return cache.favicons;
  }
  async function hasCustomFavicon() {
    try {
      await fs$1.access(path.join(customFaviconsPath, "favicon.ico"));
      return true;
    } catch (err) {
      return false;
    }
    ...
    configureServer(viteDevServer) {
      ...
      return () => {
        viteDevServer.middlewares.use(async (req, res, next) => {
          ...
          const faviconPath = fileName === "favicon.ico" && (await hasCustomFavicon()) ? path.join(customFaviconsPath, "favicon.ico") : path.join(defaultFaviconsPath, fileName);
          const mimeType = mimeTypes[path.extname(fileName)] || "application/octet-stream";
          res.writeHead(200, "OK", {
            "content-type": mimeType
          });
          res.write(await fs$1.readFile(faviconPath));
          res.end();
        });
      };
    }
...
async function extendViteConfigWithUserConfig(env, defaultConfig, userConfig) {
  ...
  if (typeof userConfig === "function") {
    debug$4("Extending vite config using user-specified function");
...
async function writeSanityRuntime(_ref4) {
  ...
  const relativeConfigLocation = studioConfigPath ? path.relative(runtimeDir, studioConfigPath) : null;
...
export { debug$3 as debug, extendViteConfigWithUserConfig, finalizeViteConfig, generateWebManifest, getViteConfig, writeSanityRuntime };
//# sourceMappingURL=runtime-r-pj3sVw.js.map
