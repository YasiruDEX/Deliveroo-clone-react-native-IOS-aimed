...
var runtime = require('./runtime-nGzgCCha.js');
...
async function buildStaticFiles(options) {
  ...
  if (extendViteConfig) {
    ...
    viteConfig = runtime.finalizeViteConfig(viteConfig);
  }
...
async function writeFavicons(basePath, destDir) {
  ...
  await fs__default.default.copyFile(path__default.default.join(destDir, "favicon.ico"), path__default.default.join(destDir, "..", "favicon.ico"));
}
...
async function startPreviewServer(options) {
  ...
  const indexPath = path__default.default.join(root, "index.html");
  let basePath;
  try {
  ...
  if (extendViteConfig) {
    previewConfig = await runtime.extendViteConfigWithUserConfig({
      command: "serve",
      mode
  ...
  if (typeof basePath === "undefined") {
    ...
    info("Using resolved base path from static build: ".concat(chalk__default.default.cyan(basePath)));
  }
...
//# sourceMappingURL=previewServer-60hj0U_i.js.map
