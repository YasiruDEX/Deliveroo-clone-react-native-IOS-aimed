...
import fs$1, { createReadStream } from 'fs';
...
const debug = debug$1("sanity:core");
  ...
  action: async (args, context) => {
    ...
    if (!destinationPath) {
      destinationPath = await prompt.single({
        type: "input",
        message: "Output path:",
        default: path.join(workDir, "".concat(dataset, ".tar.gz")),
        filter: absolutify
      });
    }
    ...
    output.print("Export finished (".concat(prettyMs(Date.now() - start), ")"));
  }
};
async function getOutputPath(destination, dataset, prompt, flags) {
  ...
  const finalPath = looksLikeFile ? dstPath : path.join(dstPath, "".concat(dataset, ".tar.gz"));
...
function parseCliFlags(args) {
  ...
  return yargs(hideBin(args.argv || process.argv).slice(2)).option("pretty", {
    type: "boolean",
    default: false
  }).option("dataset", {
    type: "string"
...
const createDocumentsCommand = {
  ...
  action: async (args, context) => {
    ...
    if (watch) {
      ...
      await fs.unlink(tmpFile).catch(noop$1);
    }
...
const description = "Downloads and validates all document specified in a workspace (beta)".concat(chalk.cyan("*"), ".\n\n").concat(chalk.cyan("*"), "Note: As it's currently in beta, some features may not be fully stable.\nWe encourage users to report any issues encountered to help us improve.\nThank you for your understanding and support!\n\nhttps://github.com/sanity-io/sanity/issues/5510\n");
...
const validateDocumentsCommand = {
  ...
  helpText: helpText$c,
  action: async (args, context) => {
    const mod = await import('./validateAction-DzRrrojC.js');
    return mod.default(args, context);
  }
};
...
function printMessage(message, context, options) {
  ...
  if (detailed) {
    ...
    output.print(inspect(JSON.parse(message.payload), {
      colors: true
    }));
  }
...
const execCommand = {
  ...
  helpText: helpText$8,
  action: async (args, context) => {
    const mod = await import('./execScript-Guh1gEsg.js');
    return mod.default(args, context);
  }
};
const corsGroup = {
...
const addCorsOriginCommand = {
  ...
  action: async (args, context) => {
    ...
    const flags = args.extOptions;
    const isFile = fs$1.existsSync(path.join(process.cwd(), origin));
    if (isFile) {
...
const listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./listApisAction-wBAuvV3D.js');
    return mod.default(args, context);
  },
  helpText: helpText$4
};
...
const deployGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deployApiAction-Xx6HzoCg.js');
    return mod.default(args, context);
  },
  helpText: helpText$3
};
...
const deleteGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deleteApiAction-SQ-0yCCw.js');
    return mod.default(args, context);
  },
  helpText: helpText$2
};
...
const listUsersCommand = {
  ...
  action: async (args, context) => {
    ...
    const ordered = sortBy(members.map(_ref3 => {
      let {
        id,
        name,
        role,
    ...
    const maxWidths = rows.reduce((max, row) => row.map((current, index) => Math.max(size(current), max[index])), sortFields.map(str => size(str)));
    const printRow = row => {
...
//# sourceMappingURL=_internal-ZiurxKnw.js.map
