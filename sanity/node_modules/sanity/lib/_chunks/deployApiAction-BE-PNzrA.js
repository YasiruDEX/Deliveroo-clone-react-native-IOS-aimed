...
var _internal = require('./_internal-XHgymXDz.js');
  ...
  print(output) {
    ...
    this.problemGroups.forEach(group => {
      group.problems.forEach(problem => {
        ...
        output.error("  ".concat(icon, " ").concat(upperFirst__default.default(problem.severity), ": ").concat(getPath(group.path)));
        output.error("  ".concat(problem.message));
        if (problem.helpId) {
          output.error("  See ".concat(generateHelpUrl.generateHelpUrl(problem.helpId)));
        }
        output.error("");
      });
    });
...
function generateTypeFilters$2(types) {
  ...
  return queryable.map(type => {
    ...
    const fields = flatten__default.default(type.fields.map(createFieldFilters$2)).filter(Boolean);
    return {
      name,
      kind: "InputObject",
...
function pluralizeTypeName(name) {
  const words = startCase__default.default(name).split(" ");
  const last = words[words.length - 1];
  const plural = pluralize__default.default(last.toLowerCase()).replace(/(\d)s$/g, "$1S");
  words[words.length - 1] = upperFirst__default.default(plural);
  return words.join("");
}
...
function generateTypeQueries$1(types, sortings) {
  ...
  queryable.forEach(type => {
    ...
    queries.push({
      fieldName: "all".concat(upperFirst__default.default(type.name)),
      filter: '_type == "'.concat(type.originalName || type.name, '"'),
      type: {
        kind: "List",
        isNullable: false,
...
async function getCurrentSchemaProps(client, dataset, tag) {
  try {
    const apiUrl = _internal.getClientUrl(client, "/apis/graphql/".concat(dataset, "/").concat(tag));
...
async function resolveApiGeneration(_ref3) {
  ...
  if (!currentGeneration) {
    ...
    _internal.debug("There is no current generation deployed, using %s (%s)", generation, specifiedGeneration ? "specified" : "default");
    return generation;
  }
  ...
  if (specifiedGeneration) {
    _internal.debug("Using specified (%s) generation", specifiedGeneration);
    return specifiedGeneration;
  }
  _internal.debug("Using the currently deployed version (%s)", currentGeneration);
...
//# sourceMappingURL=deployApiAction-BE-PNzrA.js.map
