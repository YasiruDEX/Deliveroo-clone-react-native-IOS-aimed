import { isKeyedObject, isKeySegment, isObjectSchemaType, isArrayOfObjectsSchemaType, isArraySchemaType } from '@sanity/types';
...
const remoteSnapshots = memoize((client, idPair, typeName) => {
  return memoizedPair(client, idPair, typeName).pipe(switchMap(_ref16 => {
    let {
      published,
      draft
    } = _ref16;
  ...
  const timelineStore = useMemo(() => {
    return {
      ...
      subscribe: callback => {
        ...
        tap(innerController => innerController.setLoadMore(false)), map$1(innerController => {
          var _a;
        ...
        distinctUntilChanged$1(deepCompare),
...
function resolveConfig(config) {
  ...
  workspaces.flatMap(workspaceSummary => combineLatest(workspaceSummary.__internal.sources.map(_ref18 => {
    let {
      source
    } = _ref18;
    return source;
